    ...
    if identity_file.exists():
        ...
        for line in lines:
            ...
            if found_title:
                result.append(line)
        return "\n".join(result).strip()
    return "你是 RaspiClaw，一个拥有工具调用能力的 AI 助手。"
...
class RaspiClawAgent:
    """RaspiClaw Agent with tool support and memory management."""
    ...
    def __init__(self, base_dir: Path | None = None, model: str | None = None):
        ...
        # 权限模式: full(完全权限) / ask(询问权限) / safe(安全模式)
        self.permission_mode: str = "ask"

        # 模型路由：根据 MAIN_MODEL 选择 DeepSeek、Qwen 或 OpenAI 兼容
        ...
        elif main_model == "openai_compatible":
            # 初始化 OpenAI 兼容模型（Zhipu GLM / MiniMax / 等）
            from langchain_openai import ChatOpenAI
            api_key = os.getenv("DEEPSEEK_API_KEY")
            base_url = os.getenv("DEEPSEEK_BASE_URL", "")
            model_name = model or os.getenv("DEEPSEEK_MODEL", "gpt-3.5-turbo")
    ...
    def set_permission_mode(self, mode: str) -> str:
        """设置权限模式: full(完全) / ask(询问) / safe(安全)"""
        if mode not in ("full", "ask", "safe"):
            return f"无效模式: {mode}，可用: full/ask/safe"
        self.permission_mode = mode
        all_tools = get_all_tools(self.base_dir)
        safe_tools = ["read_file", "read_skill_file", "read_rich_file", "web_search", "web_fetch"]
        if mode == "safe":
            self.tools = [t for t in all_tools if t.name in safe_tools]
        elif mode == "ask":
            self.tools = all_tools
        elif mode == "full":
            self.tools = all_tools
        return f"权限已切换为: {mode}"
...
async def interactive_mode(agent: RaspiClawAgent):
    """交互式对话模式。"""
    print("=" * 60)
    print("🐾 RaspiClaw Agent")
    print("=" * 60)
...
async def single_query(agent: RaspiClawAgent, query: str) -> str:
    """单次查询模式。"""
...
def main():
    ...
    agent = RaspiClawAgent(base_dir=base_dir)
